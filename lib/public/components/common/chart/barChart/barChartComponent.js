 ...
 * @property {Component} [placeholder="No data"] component rendered instead of the chart if data is empty
 */
...
class BarChartClassComponent {
    /**
     * Constructor
     * @param {{attrs: {data: Point[], configuration: BarChartComponentConfiguration}}} vnode the component's vnode
     */
    constructor({
        attrs: {
            data,
    ...
    handleMouseMove(event) {
        ...
        if (!this._svgNode.parentNode.contains(event.target)) {
            ...
            const { bar, relativePosition } = this._chartRenderer.getHoveredBar(relativeCursor) || {};
            this.updateHoveredPoint(bar, relativePosition ? { x: relativePosition.x + left, y: relativePosition.y + top } : null);
        }
    }
     ...
     *
     * @param {Point} [bar] the hovered bar
     * @param {Point} [absolutePosition] the absolute coordinates (relative to the page) of the point being hovered
     * @return {void}
     */
    updateHoveredPoint(bar, absolutePosition) {
        if (JSON.stringify(this.hoveredPointAbsolutePosition) !== JSON.stringify(absolutePosition)) {
            if (bar === undefined) {
                this.hoveredPointAbsolutePosition = null;
            } else {
    ...
    view() {
        return [
            createPortal(h(
                ...
                this.hoverdBar
                    ? this.configuration.tooltip(this.hoverdBar, { x: this._chartRenderer.xScale.invert(this.hoveredPointAbsolutePosition.x) })
                    : null,
            )),
            h('svg.w-100.h-100', {
 ...
 *
 * @param {Point[]} data the data to display
 ...
 */
export const barChartComponent = (data, configuration) => data.length
    ? h(BarChartClassComponent, { data, configuration })
