...
import { QualityControlFlagDetailsModel } from './details/QualityControlFlagDetailsModel.js';
...
export class QualityControlFlagsModel extends Observable {
    ...
    constructor(model) {
        ...
        this._handleFlagCreation = ({ runNumber, dataPassId, detectorId }) =>
            model.router.go(`?page=quality-control-flags&runNumber=${runNumber}&dataPassId=${dataPassId}&detectorId=${detectorId}`);

        this._handleFlagDelete = ({ runNumber, dataPassId, detectorId }) =>
            model.router.go(`?page=quality-control-flags&runNumber=${runNumber}&dataPassId=${dataPassId}&detectorId=${detectorId}`);
    }
    ...
    loadCreation({ dataPassId, runNumber, detectorId }) {
        ...
        this._creationModel = new QualityControlFlagCreationModel(
            {
                ...baseParameters,
                externalUserId: window.sessionService.session.personid,
            },
            () => this._handleFlagCreation({ runNumber, dataPassId, detectorId }),
        );
     ...
     */
    loadDetails({ flagId }) {
        this.detailsModel = new QualityControlFlagDetailsModel({
            flagId,
            externalUserId: window.sessionService.session.personid,
            onDeleteSuccess: this._handleFlagDelete,
        });
        this.detailsModel.bubbleTo(this);
    }
