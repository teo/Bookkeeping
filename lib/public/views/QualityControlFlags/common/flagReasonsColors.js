...
export const flagReasonToColorMapping = {
    ...
    LimitedAcceptanceMCReproducible: ChartColors.Brown.light,
    LimitedAcceptanceMCNotReproducible: ChartColors.Brown.dark,
    BadElectronPID: ChartColors.Red.dark,
};

/**
 * Get colour fixed to QC flag reason
 * @param {QualityControlFlagReasons} flagReason flag reason
 * @param {string} [flagReason.name] reason name
 * @param {boolean} [flagReason.bad] if no fixed colour is defined for given reasn then
 *  if flag is bad red colour will be assign, otherwise green
 * @return {string} colour in hex
 */
export const getFlagReasonColor = (flagReason) =>
    flagReasonToColorMapping[flagReason.name]
    ?? (flagReason.bad ? ChartColors.Red.dark : ChartColors.Green.dark);
