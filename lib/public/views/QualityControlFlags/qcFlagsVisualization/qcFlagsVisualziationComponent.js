 ...
 * @param {(QualityControlFlag|QualityControlFlagCreationModel)[]} flags flags
...
export const qcFlagsChartComponent = (flags, run, onPointHover) => barChartComponent(
    flags.map((flag) => flag.constructor.name === 'Object'
        ? {
    ...
    {
        placeholder: 'No QC Flags',
        tooltip: ({ flagReason, verifications, comment, y }, { x: timestamp }) => y === 'NEW*' ? 'NEW' : ('.flex-col', [
            h('.flex-row.g1', [flagColoredBadge(flagReason), `Reason: ${flagReason.method}`]),
