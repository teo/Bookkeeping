...
class StatisticsTabbedPanelModel extends TabbedPanelModel {
    ...
    constructor() {
        ...
        this._observableData = ObservableData.builder()
            ...
            .apply((remoteData) => remoteData.apply({ Success: ({ data }) => data }))
            .build();
        this._observableData.bubbleTo(this);

        this._remoteDataFetcher = new RemoteDataSource();
        this._remoteDataFetcher.pipe(this._observableData);
...
class DetectorsEfficienciesTabModel extends Observable {
    ...
    constructor() {
        ...
        this._observableDetectorsEfficiencies = ObservableData.builder()
            .initialValue(RemoteData.notAsked())
            .apply((remoteData) => remoteData.apply({ Success: ({ data }) => data }))
            .build();
        this._observableDetectorsEfficiencies.bubbleTo(this);
        this._observableDetectorsEfficiencies.observe(() => {
            this._detectors = this._observableDetectorsEfficiencies.getCurrent().apply({
                Success: (detectorEfficiency) => detectorEfficiency.reduce((detectors, { efficiencies }) => {
        ...
        this._remoteDataFetcher = new RemoteDataSource();
        this._remoteDataFetcher.pipe(this._observableDetectorsEfficiencies);
     ...
     */
    get data() {
        return this._observableDetectorsEfficiencies.getCurrent();
    }
