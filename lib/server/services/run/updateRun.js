...
const QcFlagEffectivePeriodRepository = require('../../../database/repositories/QcFlagEffectivePeriodRepository.js');
const QcFlagRepository = require('../../../database/repositories/QcFlagRepository.js');
...
exports.updateRun = async (identifier, payload, transaction) => {
    ...
    return TransactionHelper.provide(async (transaction) => {
        const runStart = runModel.timeTrgStart ?? runModel.timeO2Start ?? null;
        const { timeTrgStart, timeO2Start } = runPatch;
        const patchStart = timeTrgStart ?? timeO2Start ?? null;

        await RunRepository.update(runModel, runPatch);

        const flagIds = (await QcFlagRepository.findAll({ attributes: ['id'], where: { runNumber: runModel.runNumber } })).map(({ id }) => id);
        if (patchStart && runStart !== patchStart) {
            await QcFlagRepository.updateAll({ from: patchStart }, { where: { from: runStart } });
            await QcFlagEffectivePeriodRepository.updateAll({ from: patchStart }, {
                where: { from: runStart, flagId: { [Op.in]: flagIds } },
            });
        }
        if (patchEnd && runEnd !== patchEnd) {
            await QcFlagRepository.updateAll({ to: patchEnd }, { where: { to: patchEnd } });
            await QcFlagEffectivePeriodRepository.updateAll({ to: patchEnd }, {
                where: { to: runEnd, flagId: { [Op.in]: flagIds } },
            });
        }
