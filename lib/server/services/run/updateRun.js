...
exports.updateRun = async (identifier, payload, transaction) => {
    ...
    return TransactionHelper.provide(async (transaction) => {
        const run = await RunRepository.update(runModel, runPatch);
        ...
        const flagIds = (await QcFlagRepository.findAll({ attributes: ['id'], where: { runNumber: run.runNumber } })).map(({ id }) => id);
        if (runStart !== patchStart) {
            await QcFlagEffectivePeriodRepository.updateAll({ from: patchStart }, {
                where: { from: runStart, flagId: { [Op.in]: flagIds } },
            });
        }
        if (runEnd !== patchEnd) {
            await QcFlagEffectivePeriodRepository.updateAll({ to: runEnd }, {
