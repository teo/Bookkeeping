...
const EOR_REASON_TO_LOG = { category: 'DETECTORS', title: 'TPC' };
const EOR_REASON_LOG_TAGS = ['TPC'];
 ...
 */
const logEorReasonChange = async (runNumber, userName, transaction) => {
    const textParts = [`End of run reason for the run ${runNumber} has been changed`];

    if (userName) {
        textParts.push(`by ${userName}`);
    }

    const tags = (await getTagsByText(EOR_REASON_LOG_TAGS)).map(({ text }) => text);
 ...
 */
const updateEorReasonsOnRun = async (runId, runNumber, userName, eorReasonPatches, transaction) => {
    const reasonTypes = await ReasonTypeRepository.findAll();
    let reasonToLogId = null;

    const reasonTypesId = [];
    for (const reason of reasonTypes) {
        reasonTypesId.push(reason.id);
        if (reason.category === EOR_REASON_TO_LOG.category && reason.title === EOR_REASON_TO_LOG.title) {
            reasonToLogId = reason.id;
        }
    }

    const allReceivedReasonsExists = eorReasonPatches.every((eorReasonPatch) => reasonTypesId.includes(eorReasonPatch.reasonTypeId));
    ...
    if (needLoggingForEorReason) {
        await logEorReasonChange(runNumber, userName, transaction);
    }
