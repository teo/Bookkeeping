...
const { qualityControlFlagAdapter, qualityControlFlagVerificationAdapter } = require('../../../database/adapters');
...
class QualityControlFlagService {
    ...
    async createVerification({ qualityControlFlagId, comment, externalUserId }) {
        ...
        if (qcFlag.user.externalId === externalUserId) {
            throw new Error(`It is not possibly to verify one's own QC Flag (id:${qcFlag.id})`);
        }
        ...
        return qualityControlFlagVerificationAdapter.toEntity(newInstance);
    }
    ...
    async deleteFlag({ flagId }) {
        return await QualityControlFlagRepository.removeOne(dataSource.createQueryBuilder().where('id').is(flagId));
    }
