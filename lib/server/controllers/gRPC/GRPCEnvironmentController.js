...
class GRPCEnvironmentController {
    ...
    async Create(newEnvironment) {
        const { id, createdAt, status, statusMessage } = newEnvironment;
        return this.environmentService.create({ id, createdAt }, { status, statusMessage });
    }

    // eslint-disable-next-line jsdoc/require-jsdoc
    async Update(environmentPatch) {
        const { id: environmentId, toredownAt, status, statusMessage } = environmentPatch;
        return this.environmentService.update(environmentId, { toredownAt }, { status, statusMessage });
    }
}
