...
const listQualityControlFlagsHandler = async (req, res) => {
    ...
    if (validatedDTO) {
        try {
            const { filter, page: { limit = ApiConfig.pagination.limit, offset } = {}, sort = { createdAt: 'DESC' } } = validatedDTO.query;
...
// eslint-disable-next-line valid-jsdoc
/**
 * Create QualityControlFlagVerification
 */
const createQualityControlFlagVerificationHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.bodyOnly({
    ...
    if (validatedDTO) {
        try {
            const createdVerification = await qualityControlFlagService.createVerification(validatedDTO.body);
            res.status(201).json({ data: createdVerification });
        } catch (error) {
            updateExpressResponseFromNativeError(res, error);
        }
    }
 ...
 * @param {*} res 
 */
const deleteQualityControlFlagHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
    ...
    if (validatedDTO) {
        try {
            const result = await qualityControlFlagService.deleteFlag(validatedDTO.params);
...
exports.QualityControlFlagsController = {
    ...
    createQualityControlFlagVerificationHandler,
    deleteQualityControlFlagHandler,
};
