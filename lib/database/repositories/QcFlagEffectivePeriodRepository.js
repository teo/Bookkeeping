...
class QcFlagEffectivePeriodRepository extends Repository {
    ...
    async findGaqEffectivePeriods({ dataPassId, runNumbers }) {
        const gaqPeriodsSubQuery = `
            ...
            FROM (
                    (
                        SELECT data_pass_id,
                            run_number,
                            COALESCE(UNIX_TIMESTAMP(COALESCE(qcfep.\`from\`, run.time_trg_start, run.time_o2_start )), 0) AS timestamp
                        FROM quality_control_flag_effective_periods AS qcfep
                            ...
                            INNER JOIN runs as run on run.run_number = qcf.run_number
                    )
                    UNION
                    (
                        SELECT data_pass_id,
                            run_number,
                            UNIX_TIMESTAMP(COALESCE(qcfep.\`to\`, run.time_trg_end, run.time_o2_end, NOW())) AS timestamp
                        FROM quality_control_flag_effective_periods AS qcfep
                            INNER JOIN quality_control_flags AS qcf ON qcf.id = qcfep.flag_id
                            INNER JOIN data_pass_quality_control_flag AS dpqcf ON dpqcf.quality_control_flag_id = qcf.id
                            INNER JOIN runs as run on run.run_number = qcf.run_number
        ...
        FROM (${gaqPeriodsSubQuery}) AS gaq_periods
        INNER JOIN global_aggregated_quality_detectors AS gaqd
            ON gaq_periods.data_pass_id = gaqd.data_pass_id
            AND gaqd.run_number = gaq_periods.run_number
        INNER JOIN runs as run on run.run_number = qcf.run_number
        LEFT JOIN (
            quality_control_flag_effective_periods AS qcfep
        ...
        ) ON COALESCE(UNIX_TIMESTAMP(COALESCE(qcfep.\`from\`, run.time_trg_start, run.time_o2_start )), 0) <= gaq_periods.\`from\`
            AND gaq_periods.\`to\` <= UNIX_TIMESTAMP(COALESCE(qcfep.\`to\`, run.time_trg_end, run.time_o2_end, NOW()))
            AND gaqd.dpl_detector_id = qcf.dpl_detector_id
                AND gaq_periods.run_number = qcf.run_number
                AND dpqcf.data_pass_id = gaq_periods.data_pass_id
