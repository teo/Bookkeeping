...
module.exports = () => {
    ...
    it('should successfully create a log when updating EoR reason to DETECTOR-TPC', async () => {
        const runNumber = 1;
        ...
        expect(lastLog.title).to.equal('EoR reason has changed for 1');
        expect(lastLog.text.startsWith('End of run reason for the run 1 has been changed')).to.be.true;
        expect(lastLog.tags).to.lengthOf(0);
    });

    it('should successfully NOT create a log when updating EoR reason for something else than DETECTOR-TPC', async () => {
        const runNumber = 1;

        await runService.update(
            { runNumber },
            { relations: { eorReasons: [{ reasonTypeId: 1, description: 'A description' }] } },
        );
